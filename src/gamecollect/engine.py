...
class CollectorEngine:
    ...
    def _live_supersedes_cooldown_fallback(
        ...
        Thin wrapper over the shared :meth:`_live_supersedes_captured` policy — see
        ...
        """
        return CollectorEngine._live_supersedes_captured(applied, fallback)
    ...
    def _live_supersedes_captured(applied: NormalizedMatch, fallback: NormalizedMatch) -> bool:
        ...
        Both the cooldown fallback (:meth:`_live_supersedes_cooldown_fallback`) and
        the resumption-tracker fallback (:meth:`_live_supersedes_fallback`) are judged
    ...
    def _reconcile_live_apply(self, match_id: str, applied: NormalizedMatch) -> None:
        ...
        if (
            ...
            or self._live_supersedes_captured(applied, fallback)
        ):
    ...
    def _live_supersedes_fallback(applied: NormalizedMatch, fallback: NormalizedMatch) -> bool:
        ...
        Thin wrapper over the shared :meth:`_live_supersedes_captured` policy (Z2).
    ...
    def _accrue_fallback(old: NormalizedMatch | None, new: NormalizedMatch) -> NormalizedMatch:
        ...
        * PAYLOAD: preserve-richer merge (:func:`is_empty_payload_value`) — the
        ...
        minute, display_clock = CollectorEngine._paired_clock(winner, other, other_vetoed=vetoed)
        ...
        if winner_authoritative:
            ...
            kickoff_utc = (
                winner.kickoff_utc if winner.kickoff_utc is not None else other.kickoff_utc
            )
        else:

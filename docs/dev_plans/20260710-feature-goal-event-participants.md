...
- **Own-goal scorer attribution is inverted relative to a normal goal**: for `own_goal`, `participants[0]` is the player who scored into their own net, and `event.team` is the **benefiting** team — not that player's own team. (Corrected post-completion by commit `aea862c`, which fixed this claim after it was originally documented backwards; see fixture evidence — Diney Borges/Cabo Verde scores, row carries `team: Argentina`; Mohamed Manai/Qatar scores, row carries `team: Canada`.) `payload.scorer` on an `own_goal` row therefore names a player under the "wrong" (own, conceding) team relative to `payload.team` by normal-goal convention. Documented explicitly in Requirements/DESIGN.md rather than special-cased away, since the underlying `NormalizedEvent.team`/`.player` semantics for own goals are pre-existing behavior this plan does not change.
...
- `_stored_events` (`engine.py:862-889`) is updated symmetrically: it must read back `extras.get("scorer")`/`extras.get("assist")` when reconstructing a `goal`/`own_goal` row, and `extras.get("player")`/`extras.get("assist")` for every other `event_type`, so the resumption/baseline-reconstruction path reconstructs an equivalent `NormalizedEvent` for both branches. This is not optional/deferred — it lands in the same commit as `_event_to_row`. (Post-completion, commit `6dcde1a` added a further legacy-payload fallback: a goal-family row written before this convention landed, lacking `payload.scorer` entirely, now falls back to reading `payload.player` instead of reconstructing `player=None`. Both functions' event_type→payload-key mapping was later centralized in a shared `_participant_key()` helper by commit `b5a0bd5` — see Final Results.)
...
### Post-completion review-gauntlet rounds

Several `/review-gauntlet` rounds landed on top of the Phase 1 commit before merge, all within this plan's existing scope (no new phases):

- `6dcde1a` — round 1: `_stored_events` gained a legacy-payload fallback (goal-family rows written before this branch landed, lacking `payload.scorer`, now read back via `payload.player` instead of silently losing the scorer on restart — this refines the "reads back `extras.get("scorer")`" line in Requirements above, which predates the fallback). `_GOAL_FAMILY_EVENT_TYPES` extracted as a shared module-level constant (previously an inline literal in both `_event_to_row`/`_stored_events`). `_event_to_row`'s "Core-agnostic and lossless" docstring claim qualified to acknowledge the goal-family gate as a deliberate, visible boundary erosion.
- `3d68557` — round 3: added a taxonomy-parity test guarding `_GOAL_FAMILY_EVENT_TYPES` against silent drift from the football pack's own `goal`/`own_goal` taxonomy declarations. A second finding (persisted legacy payloads not normalized on disk, only in-memory `_stored_events` reconstruction) was quarantined — fixing it would require a real migration or public-read-path normalization, both of which conflict with this plan's "No new schema, no migration" Architecture Decision.
- `b64202e` — round 5: hedged the own-goal team-attribution claim in `docs/DESIGN.md`/README as unverified ESPN provider behavior rather than definite fact, consistent with the adjacent ESPN-assumptions hedging already in Requirements.
- `b5a0bd5` — further round: extracted a `_participant_key(event_type) -> str` helper (module-level, next to `_GOAL_FAMILY_EVENT_TYPES`) so the goal-family→payload-key mapping described in Requirements above is derived in exactly one place, called from both `_event_to_row` and `_stored_events` — eliminating the inline-duplication drift risk 3 independent review angles flagged. Also documented the taxonomy-parity test's football-pack-specific limitation, and fixed a falsy-empty-string edge case in `tests/test_replay.py`'s `_event_player` test helper (an `or`-chain collapse had changed its `str | None` contract to potentially return `""`).
- The legacy `payload.player` false-drift-on-restart issue quarantined in round 3 was independently rediscovered by round `b5a0bd5`'s adversarial Codex gate with concrete evidence (writer fingerprint comparison bypasses the read-time fallback) and re-quarantined against the same Architecture Decision — not a new finding, no action taken.
- A standalone `/codex:adversarial-review` pass (post-`b5a0bd5`, not a `/review-gauntlet` round) narrowed and closed **half** of the round-3 quarantine: it found that `client.get_events_since`/`events --json` — the actual public read boundary, not just internal engine reconciliation — exposed pre-rename `payload.player` goal rows in the old shape indefinitely, since `_stored_events`'s legacy fallback only runs on the engine's own resumption path. **Fixed** (not quarantined): `client.py` gained its own `_normalize_legacy_payload` read-time rewrite (mirroring `_stored_events`'s fallback) in `Event.from_row`, plus a second, deliberate copy of `_GOAL_FAMILY_EVENT_TYPES` (avoiding an import of the daemon-weight `engine` module into the lightweight read client), kept from drifting by `test_client_goal_family_matches_engine`. This is schema/migration-free — a decoded-dict rewrite at read time — so it does not conflict with "No new schema, no migration." The other half of the round-3 quarantine (the writer's raw-JSON-fingerprint comparison causing false-positive `SequenceError`/drift-detection spam on the *write* path when a legacy row is re-polled) is untouched and remains quarantined — a distinct mechanism (write-path idempotency, not public-read shape) that this fix does not address.
- `aea862c` — a further standalone `/codex:adversarial-review` pass (post-`7b4be12`) found the own-goal team-attribution claim added by round 5 (`b64202e`) was stated **backwards**: `docs/DESIGN.md` claimed `event.team` on an `own_goal` row is the team that did *not* benefit, but the checked-in fixtures show it's the **benefiting** team (Diney Borges/Cabo Verde scores, row carries `team: Argentina`; Mohamed Manai/Qatar scores, row carries `team: Canada`). **Fixed**: corrected `docs/DESIGN.md`'s caveat text, and added `tests/football/test_espn_event_mapping.py::TestOwnGoalTeamAttribution` pinning scorer/team semantics against the real `qatar_canada_760440` own-goal fixture. This plan's own Context (line 39) and Requirements (line 58) sections carried the same backwards claim and are corrected in this changelog pass, per a subsequent `/code-review` finding that flagged the plan file as not updated alongside `aea862c`.

...
- **Second Codex adversarial review (post-/deep-review, FIXED)**: (1) High — the seq-drift guard compared only `type`, so a same-type shifted event (goal replayed over a different goal) was accepted as idempotent replay, keeping the stale row and duplicating the old one at the next seq (Codex reproduced it). `append_events` now compares the full event fingerprint (`type` + every event column, payload in its canonical encoding) and raises `SequenceError` on any mismatch. (2) Medium — valid JSON with a wrong top-level type (e.g. `[]`) hit `assert_espn_*_shape` before the converting try and escaped as raw `TypeError`; both assertions now reject non-dict responses with `ShapeDriftError` and run inside the seam's converting try. 4 regression tests (191 passed, 2 skipped).
- **Codex adversarial review (post-/code-review, FIXED)**: (1) High — `_assert_match_owned` allowed an absent match row, so source B could append events for bare `match_id` before source A seeded it; once A owned the match, B's rows stayed visible through the source-blind `get_events_since`. Child writes (`append_events`, `map_provider_match`) now require a seeded, owned match row (`UnseededMatchError`); the former `test_append_events_before_match_seeded_is_allowed` soft-ref test is inverted. (2) Medium — `append_events(match_id, events)` never checked per-row `match_id` against the batch argument, silently re-homing mixed batches; rows declaring a different `match_id` now raise `ValueError`. 4 regression tests added (179 passed, 2 skipped). Consequence for the interfaces plan: the engine must call `upsert_match` (or `register_unreconciled_match`, which seeds a qualified strip row) before appending events. (The skip-path sharp edge — a qualified id returned WITHOUT seeding — was subsequently fixed by the /code-review pass below: the skip path now returns `None`.)
- **/code-review medium (post-adversarial, FIXED — 8 findings)**: (1) positional event seqs are not a stable idempotency key across re-polls — `append_events` now compares re-sent seqs' `type` against the stored row and raises `SequenceError` on mismatch (loud drift detection; ESPN keyEvent ids can't replace positional seqs — non-numeric/synthetic ids exist and ordering isn't guaranteed); (2) null `participants` entries no longer abort the whole summary (isinstance guard, mirroring the rosters loop); (3) `normalize_status` covers both "Half Time" and "Halftime" (interval literal was never live-verified); (4) `register_unreconciled_match(conn, writer, match)` returns `None` on the missing-identity skip path (a `str` return always names a seeded row) and (5) read-merge-writes `payload` instead of erasing keys wholesale; (6) resolution step (b) now caches its bind in `provider_match_map` (docstring said (b)/(c) cache — code didn't); (7) HT scores are parsed from competitor `linescores` (were promised in payload but unconditionally null); (8) reader imports `_BUSY_TIMEOUT_MS` from connection.py instead of re-declaring it. 7 regression tests added (186 passed, 2 skipped). Verified-REFUTED (no change): `MAX(seq)` pre-read TOCTOU (same-source concurrency is documented-unsupported, outcome bounded); "dead" API-Football/football-data status tables (pinned for gamealerts parity).
